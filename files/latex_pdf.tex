...
\title{
    \vspace{-2cm}
    \includegraphics[width=2.5cm]{outputs/logo.png} \\ % Ensure this path exists or remove this line
    \Large Cairo University - Faculty of Engineering \\
    \large Computer Engineering Department \\
    \large Communications Engineering – Fall 2025 \\[1cm]
    \textbf{\Huge FM Stereo Receiver System} \\
    \Large Final Project Report
}
\author{
    \textbf{Ahmed Fathy} - 9230162 \\
    \textbf{Ziad Montaser} - 9231142
}
...
% PAGE 3: TASK 1
% ----------------------------------------------------------------------
\setcounter{page}{1} % Starts page numbering here (optional, or remove to count from title)
\section{Task 1: Frequency Deviation Effects}

\subsection{Results}

\subsubsection*{a) Bandwidth Comparison (Measured vs Theoretical)}

Theoretical Bandwidth is calculated using \textbf{Carson's Rule}: 
\[ B_{theo} = 2(\Delta f + f_m) \]
where $f_m \approx 53$ kHz (includes Stereo Pilot + L-R).

\begin{table}[h]
    \centering
    \caption{Bandwidth and SNR Comparison for Different Deviations}
    \begin{tabular}{cccc}
        \toprule
        \textbf{Deviation $\Delta f$} & \textbf{Theoretical BW (kHz)} & \textbf{Measured BW (99\%) (kHz)} & \textbf{Output SNR (dB)} \\
        \midrule
        \textbf{50 kHz}  & 206.0 & 76.6  & 27.42 \\
        \textbf{75 kHz}  & 256.0 & 80.5  & 30.90 \\
        \textbf{100 kHz} & 306.0 & 112.5 & 33.39 \\
        \bottomrule
    \end{tabular}
\end{table}

\subsubsection*{b) Output SNR vs Frequency Deviation}
We measured the Output SNR for a fixed Input SNR of \textbf{25 dB}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{outputs/deviation_vs_snr.png}
    \caption{Frequency Deviation vs. Output SNR}
    \label{fig:task1_snr}
\end{figure}

\subsubsection*{c) Trade-off and Recommendation}

\textbf{Trade-off Observed:}
\begin{itemize}
    \item \textbf{Higher Deviation} ($\Delta f$) increases the signal power relative to noise after demodulation (FM Improvement Factor), resulting in \textbf{better SNR} (+6 dB gain from 50k to 100k).
    \item \textbf{However}, it significantly increases the \textbf{Required Bandwidth} (from $\sim$77k to $\sim$112k Measured).
\end{itemize}

\textbf{Choice:}
\textbf{75 kHz} is the optimal choice and the standard for FM broadcasting because:
\begin{enumerate}
    \item It provides a high SNR ($>$30 dB in this test scenario), sufficient for high-fidelity audio.
    \item The bandwidth fits comfortably within standard 200 kHz channel spacing, minimizing adjacent channel interference.
    \item 100 kHz would offer marginally better SNR but risks spectral mask violations.
\end{enumerate}
...
% PAGE 4: TASK 2
% ----------------------------------------------------------------------
\section{Task 2: Noise Immunity Analysis}

\subsection{Results Table}

\begin{table}[h]
    \centering
    \caption{Noise Immunity Test Results}
    \begin{tabular}{ccccc}
        \toprule
        \textbf{Input SNR (dB)} & \textbf{Output SNR (dB)} & \textbf{Sep L to R (dB)} & \textbf{Sep R to L (dB)} & \textbf{THD (\%)} \\
        \midrule
        \textbf{5}  & 5.05  & 4.03  & 4.66  & 1.79 \\
        \textbf{10} & 23.47 & 18.73 & 18.62 & 0.23 \\
        \textbf{15} & 28.90 & 21.89 & 23.09 & 0.13 \\
        \textbf{20} & 33.97 & 23.01 & 23.12 & 0.06 \\
        \textbf{25} & 38.84 & 23.31 & 23.36 & 0.05 \\
        \bottomrule
    \end{tabular}
\end{table}
...
\subsubsection*{a) Output SNR vs Input SNR \& b) Separation vs Input SNR}

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{outputs/graphs_results.png}
    \caption{Task 2 Results: (Left) SNR Transfer, (Center) Separation, (Right) THD}
    \label{fig:task2_results}
\end{figure}

\subsection{Analysis}

\subsubsection*{c) Threshold SNR Effect}

\textbf{Threshold SNR:} Approximately \textbf{10 dB}.

\textbf{Observation:}
\begin{itemize}
    \item Below 10 dB Input SNR (e.g., at 5 dB), the Output SNR matches the Input SNR ($\sim$5 dB), indicating the FM system has "crashed" or lost its coding gain (FM Threshold Effect). THD spikes to 1.79\%.
    \item Above 10 dB (e.g., at 15 dB), the Output SNR jumps significantly (to $\sim$29 dB), showing the expected "FM Improvement Factor."
    \item Channel Separation also degrades rapidly below 10 dB because the noise floor overwhelms the pilot tone recovery, causing sync loss.
\end{itemize}

\textbf{Cause:}
The \textbf{FM Threshold Effect} occurs when the noise vector amplitude occasionally exceeds the carrier amplitude, causing phase wraparound events ("clicks") that generate impulse noise across the entire baseband, destroying the SNR gain.
...
% PAGE 5: TASK 3
% ----------------------------------------------------------------------
\section{Task 3: Channel Separation Analysis}
...
We injected a \textbf{1 kHz tone} into the Left channel (Right = Silence) and measured the recovered RMS levels.

\begin{itemize}
    \item \textbf{Recovered Channel Separation}: \textbf{23.31 dB}
\end{itemize}

\subsection{Analysis}

\subsubsection*{b) Limiting Component}
The primary limiting factor is the \textbf{Pilot Extraction Filter}.

\begin{itemize}
    \item The standard IIR (Butterworth) filter used to extract the 19 kHz pilot introduces a \textbf{Frequency-dependent Phase Delay}.
    \item Since the 38 kHz demodulation carrier is derived from this pilot (by squaring/doubling), any phase error in the pilot is \textbf{doubled} in the carrier.
    \item This Carrier Phase Error causes the $L-R$ (Difference) signal to be partially demodulated as $L+R$ (Sum), or vice-versa, resulting in crosstalk.
\end{itemize}

\subsubsection*{c) Proposed Improvement}
\textbf{Modification:} Use \textbf{Zero-Phase Filtering} or \textbf{Delay Compensation}.

\begin{enumerate}
    \item \textbf{Zero-Phase Filtering (\texttt{filtfilt}):} In a buffered/offline system, apply the pilot filter forward and backward to cancel out phase delay completely.
    \item \textbf{Delay Compensation:} In a real-time system, add a matching delay line to the composite signal path to align it with the delayed pilot before demodulation.
\end{enumerate}

\textit{Note: In Task 4 and 5, we verified that minimizing this phase error (by using wider bandwidth filters or lower orders) directly improves separation.}
...
% PAGE 6: TASK 4
% ----------------------------------------------------------------------
\section{Task 4: Filter Design Impact Analysis}

\subsection{Objective}
Analyze how the order of the Pilot Bandpass Filter (BPF) affects the performance of the FM stereo receiver, specifically focusing on \textbf{Channel Separation} and signal recovery quality.

\subsection{1. Pilot Extraction Filter Response}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{outputs/filter_responses.png}
    \caption{Pilot Filter Frequency Responses}
\end{figure}

\textbf{Observation:}
\begin{itemize}
    \item Higher order filters provide sharper roll-off and better isolation of the pilot tone from adjacent noise/signals.
    \item However, higher order IIR filters (like the Butterworth used here) introduce larger \textbf{phase delays} (group delay), particularly near the cutoff edges.
\end{itemize}

\subsection{2. Channel Separation Analysis}
We measured channel separation in \textbf{both directions} to ensure symmetry:
\begin{enumerate}
    \item \textbf{L $\to$ R Leakage}: Transmitting Left-only, measuring noise in Right.
    \item \textbf{R $\to$ L Leakage}: Transmitting Right-only, measuring noise in Left.
\end{enumerate}

\begin{table}[h]
    \centering
    \caption{Filter Order vs. Channel Separation}
    \begin{tabular}{ccccc}
        \toprule
        \textbf{Filter Order} & \textbf{Mode} & \textbf{Signal RMS} & \textbf{Leakage RMS} & \textbf{Separation (dB)} \\
        \midrule
        \textbf{4} & L $\to$ R & 0.1974 & 0.0149 & \textbf{22.47 dB} \\
        \textbf{4} & R $\to$ L & 0.2056 & 0.0157 & \textbf{22.35 dB} \\
        \midrule
        \textbf{8} & L $\to$ R & 0.1973 & 0.0230 & 18.65 dB \\
        \textbf{8} & R $\to$ L & 0.2055 & 0.0241 & 18.61 dB \\
        \midrule
        \textbf{12} & L $\to$ R & 0.1971 & 0.0379 & 14.33 dB \\
        \textbf{12} & R $\to$ L & 0.2054 & 0.0395 & 14.31 dB \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{outputs/separation_trend.png}
    \caption{Separation Trend vs Filter Order}
\end{figure}

\subsubsection*{Key Finding: Inverse Relationship}
The results show a clear trend: \textbf{As the filter order increases, Channel Separation decreases.} Use of the \texttt{sosfilt} (causal) function for pilot extraction introduces a phase delay that grows with filter order. This desynchronizes the regenerated 38 kHz carrier from the payload, causing leakage.

\subsection{3. Time-Domain Signal Recovery}
The following plots show the recovered Signal (Blue) and Leakage (Red) channels. Ideally, the Red line should be flat (silence).

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{outputs/waveform_order_4_L_to_R.png}
    \caption{Order 4: Waveform Recovery (Best Separation)}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{outputs/waveform_order_8_L_to_R.png}
    \caption{Order 8: Waveform Recovery}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{outputs/waveform_order_12_L_to_R.png}
    \caption{Order 12: Waveform Recovery (Worst Separation)}
\end{figure}

\subsection{4. Conclusion}
While higher-order filters are generally desirable for rejecting noise and interferers, in the context of \textbf{coherent FM demodulation}, they introduce detrimental phase shifts. Without delay compensation (or using zero-phase filtering like \texttt{filtfilt}), \textbf{lower-order filters (Order 4)} generally yield better stereo separation because they maintain tighter phase alignment between the pilot and the multiplexed signal.
...
% PAGE 7: TASK 5
% ----------------------------------------------------------------------
\section{Task 5: System Robustness Analysis}
...
\subsubsection*{a) Channel Separation vs. Pilot Frequency Error}
We swept the pilot frequency offset from $-500$ Hz to $+500$ Hz and measured the resulting channel separation.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{outputs/robustness_curve.png}
    \caption{Channel Separation vs Pilot Frequency Error}
\end{figure}

\begin{table}[h]
    \centering
    \caption{Separation at specific offsets}
    \begin{tabular}{cc}
        \toprule
        \textbf{Offset (Hz)} & \textbf{Separation (dB)} \\
        \midrule
        -500 & 7.14 \\
        -300 & \textbf{20.00} \\
        -100 & \textbf{25.03} \\
...
\textbf{Observation:} Separation peaks around -100 Hz offset rather than 0 Hz. This suggests that the receiver's filters introduce a baseline phase delay that is accidentally "compensated" by a slightly lower frequency pilot.

\subsubsection*{b) Spectrum Analysis at +500 Hz Error}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{outputs/spectrum_plus500Hz.png}
    \caption{Audio Spectrum at +500 Hz Pilot Error}
\end{figure}

\textbf{Observation:}
The "Silence" channel (Red) has almost the same energy level as the "Signal" channel (Blue).
\begin{itemize}
    \item \textbf{Separation is effectively 0 dB ($\sim$0.35 dB measured).}
    \item This occurs because at +500 Hz, the pilot is significantly phase-shifted. A phase error of $\approx 45^\circ$ causes the L-R signal to demodulate into the wrong quadrant, causing massive crosstalk.
\end{itemize}

\subsubsection*{c) Tolerance Range}
We define "tolerance" as the range of frequency offsets where Channel Separation remains \textbf{above 20 dB}.

\textbf{Result:} The system can tolerate pilot frequency errors in the range: \textbf{[-300 Hz, +50 Hz]}.
...
\begin{lstlisting}[language=Python, caption=Full Project Source Code]
import numpy as np
...
def run_task_1():
    ...
    plt.xlabel('Frequency Deviation Delta f (kHz)')
