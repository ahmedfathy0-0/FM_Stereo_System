...
if __name__ == "__main__":
    ...
    plt.plot(OFFSETS, res, 'o-')
    plt.axhline(20, c='r', ls='--')
    plt.title('Separation vs Pilot Frequency Error')
    plt.xlabel('Pilot Frequency Offset (Hz)')
    ...
    if bad_audio:
        ...
        plt.plot(f_l, s_l, 'b', alpha=0.7, label='Left Channel (blue)')
        plt.plot(f_r, s_r, 'r', alpha=0.7, label='Right Channel (red)')
        plt.title('Spectrum @ +500 Hz Error')
        plt.xlabel('Frequency (Hz)')
        plt.ylabel('Magnitude (dB)')
        plt.xlim([0, 15000])
        plt.ylim([-80, 5])
        plt.xticks(np.arange(0, 15001, 1000))
        plt.yticks(np.arange(-80, 6, 5))
        plt.legend()
        plt.grid(True)
        plt.savefig(os.path.join(OUTPUT_DIR, 'spectrum_plus500Hz.png'))
        plt.close()
